...
define("MOODLE_COURSEROLE_STUDENT" , 9); 			 // moodle:student
define("MOODLE_COURSEROLE_INSTRUCTOR" , 5);  	// moodle:teacher
define("MOODLE_COURSEROLE_ASSISTANT"  , 7);  	// moodle:non-editing-teachers
define("MOODLE_COURSEROLE_GUEST", 11); // moodle:gast
...
public static function isenroled_parameters() {
    ...
    public static function isenroled($request) {
    	...
    	try {
		    	...
		    	syncLog("role found : $ret (strlen: ".strlen($ret).") ");
		    	synclog("ret = ".$ret);
		    	$status = false;
		    	...
		    	if ($role == 'non-editing-teacher' || $role == 'non-editing teacher' && ($ret == 'non-editing teacher' or $ret == 'non-editing-teacher')) {
		    		#$status = true;
		    		syncLog("Special: $role found, interpreted as 'non-editing teacher' ");
		    	}
		    	if ($ret == "" || $ret == " ") {
		    		$status = false;
		    	syncLog("*** NO ENROLMENTS FOUND!");
		    	}
			} catch (Exception $e) {
				syncLog("Error while parsingincomings arguments!");
				syncLog($e->getMessage());
 ...
 public static function courseuser_update_parameters() {
  ...
  /**
    ...
    #	$role = $params["courserole"];
    	...
    	if ($role == 9) { $role_2 = "student";}
    	if ($role == 5) { $role_2 = "teacher";}
    	if ($role == 7) { $role_2 = "non-editing teacher";}
    	if ($role == 11) { $role_2 = "guest";}
